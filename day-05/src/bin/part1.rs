...
fn process(input: &str) -> u32 {
    let mut seeds = HashMap::<u64, [u64; 7]>::new();
    ...
    for (key, seed) in seeds.iter_mut() {
        let mut result;
        ...
        if let Some(value) = result {
            seed[seed_index] = search_value - value.1 + value.0;
        } else {
            seed[seed_index] = search_value;
        }

        // 2- soil to fertilizer
        search_value = seed[seed_index];
        seed_index = 1;
        ...
        if let Some(value) = location {
            location = Some(value.min(seed[6].try_into().unwrap()));
        } else {
            location = Some(seed[6].try_into().unwrap());
        }
    }
