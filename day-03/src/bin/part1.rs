...
fn process(input: &str) -> u32 {
    let mut valid_number: Vec<NumberScheme> = Vec::new();
    ...
    for scheme in symbol_array {
        ...
        if scheme.line > 0 {
            let mut line = scheme.line - 1;
            let mut position_1 = if scheme.indexe == 0 {
                0
            } else {
                scheme.indexe - 1
            };
            let mut position_2 = scheme.indexe;
            let mut position_3 = scheme.indexe + 1;
            ...
            for element in collect {
                if !valid_number
                    .iter()
                    .any(|&x| x.line == element.line && x.indexes[0] == element.indexes[0])
                {
                    valid_number.push(element.clone());
                }
            }
        }
    ...
    let mut sum: u32 = 0;
    for scheme in valid_number.clone() {
        println!(
            "Valid: {} on line {} and with indexes {} to {}",
            scheme.number, scheme.line, scheme.indexes[0], scheme.indexes[1]
        );
        sum += scheme.number;
    }
    return sum;
